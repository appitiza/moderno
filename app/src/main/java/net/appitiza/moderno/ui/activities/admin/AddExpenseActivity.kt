...
class AddExpenseActivity : BaseActivity(), UserSiteClick {
    ...
    private fun setClick() {
        ...
        spnr_admin_add_expense_site.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
        ...
        spnr_admin_add_expense_category.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            override fun onNothingSelected(parent: AdapterView<*>?) {
        ...
        tv_admin_submit_add_expense.setOnClickListener { addExpense() }
        ...
        db.collection(Constants.COLLECTION_SITE)
                ...
                .addOnCompleteListener { fetchall_task ->
                    ...
                    if (fetchall_task.isSuccessful) {
                        ...
                        spnr_admin_add_expense_site.adapter = siteAdapter
                        mProgress?.dismiss()
        ...
        db.collection(Constants.COLLECTION_EXPENSE_CATEGORY)
                ...
                .addOnCompleteListener { fetchall_task ->
                    ...
                    if (fetchall_task.isSuccessful) {
                        ...
                        spnr_admin_add_expense_category.adapter = categoryAdapter
                        mProgress?.dismiss()
    ...
    private fun addExpense() {
        if (validate()) {
            mProgress?.setTitle(getString(R.string.app_name))
            mProgress?.setMessage(getString(R.string.adding_expense_details))
            mProgress?.setCancelable(false)
            mProgress?.show()
            ...
            db.collection(Constants.COLLECTION_INCOME_EXPENSE)
                    ...
                    .addOnCompleteListener { add_task ->
                        if (add_task.isSuccessful) {
                            mProgress!!.dismiss()
                            Utils.showDialog(this, getString(R.string.expense_addded))
                            et_admin_add_expense_payment.setText("")
                            et_admin_add_expense_reason.setText("")
    ...
    private fun validate(): Boolean {
        return if (selectedSite.siteid == null) {
            ...
            false
        } else if (TextUtils.isEmpty(et_admin_add_expense_payment.text.toString())) {
            showValidationWarning(getString(R.string.payment_missing))
            false
        } else if (TextUtils.isEmpty(et_admin_add_expense_reason.text.toString())) {
